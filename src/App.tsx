...
import { Calculator, Home, TrendingUp, RefreshCw, Menu, X, Calendar } from 'lucide-react'; 
...
const tabs = [ 
  ...
  { id: 'age', label: 'Age', icon: Calendar, color: 'text-neon-pink', border: 'neon-border-pink', bg: 'bg-neon-pink' }, 
          ...
          {tabs.map((tab) => ( 
            <button 
              ...
              className={cn( 
                ...
                activeTab === tab.id ? "bg-zinc-800/50 text-white border border-zinc-700 shadow-xl" : "text-zinc-500 hover:text-white" 
              )} 
            > 
              <div className={cn( 
          ...
          <header className="flex justify-between items-end mb-12 border-b border-zinc-900 pb-8"> 
            ...
            <div className="hidden lg:flex gap-2"> 
              {['CYAN', 'PURPLE', 'LIME', 'PINK'].map((color, i) => ( 
                <div key={color} className={cn("w-8 h-1", [ 'bg-neon-cyan', 'bg-neon-purple', 'bg-neon-lime', 'bg-neon-pink' ][i])} /> 
              ))} 
            </div> 
          </header> 
